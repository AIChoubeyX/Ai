...
const Community = () => {
  ...
  return (
    <div className="flex-1 h-full flex flex-col gap-4 p-6 bg-gray-900">
      <h1 className="text-xl font-semibold mb-4 text-white">Community Creations</h1>

      {fatalError && (
        <p className="text-red-400 bg-red-900/30 p-2 rounded-md">
          {fatalError.message || "Something went wrong while fetching creations."}
        </p>
      )}

      <div className="bg-gray-800 h-full w-full rounded-xl overflow-y-scroll flex flex-wrap">
        {loading ? (
          <p className="text-center text-gray-400 py-10 w-full">Loading...</p>
        ) : creations.length === 0 ? (
          <p className="text-center text-gray-500 py-10 w-full">
          ...
          creations.map((creation, index) => (
            ...
            >
              ...
              />
              <div className="absolute bottom-0 top-0 right-0 left-3 flex gap-2 items-end justify-end group-hover:justify-between p-3 group-hover:bg-gradient-to-b from-transparent to-black/80 text-white rounded-lg">
                <p className="text-sm hidden group-hover:block text-white/90">
                  {creation.prompt}
                </p>
                <div className="flex items-center gap-2">
                  <button 
                    onClick={() => downloadImage(creation.content)}
                    className="hidden group-hover:block p-1 text-gray-300 hover:text-white transition-colors"
                    title="Download image"
                  >
                    <Download className="w-4 h-4" />
                  </button>
                  <p className="text-white/90">{creation.likes.length}</p>
                  <Heart
                    onClick={() => imageLikeToggle(creation.id)}
