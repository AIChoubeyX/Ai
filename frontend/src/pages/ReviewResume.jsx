...
const ReviewResume = () => {
  ...
  const handleCopyContent = async () => {
    try {
      ...
      setCopied(true);
      toast.success("Resume analysis copied to clipboard!");
      ...
      >
        ...
        <div className="relative mt-2">
          <input
            ...
            required
          />
          <div className="w-full p-3 flex items-center justify-between bg-gray-700 border border-gray-600 rounded-md hover:bg-gray-600 transition-colors cursor-pointer">
            <div className="flex items-center gap-3">
              <Upload className="w-4 h-4 text-gray-400" />
              <span className="text-sm text-gray-300">
                {input ? input.name : "Choose File"}
              </span>
            </div>
        ...
        <button
          disabled={loading}
          className="w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#00DA83] to-[#009BB3] text-white py-2 px-4 mt-6 text-sm rounded-lg cursor-pointer hover:opacity-90 transition-opacity disabled:opacity-50"
        >
          {loading ? (
